...
def load_model():
    ...
    return pipeline(
        ...
        framework="pt"  # ← ADD THIS LINE - forces PyTorch instead of TensorFlow
    )

# Try to load model and handle errors gracefully
try:
        ...
        if not review_text or len(review_text.strip()) < 3:
            ...
            with st.spinner("Analyzing..."):
                ...
                if result:
                    ...
                    st.error("⚠️ Unable to analyze the review. Try a different input.")
        ...
        try:
            ...
            if not review_col:
                ...
                if st.button("🚀 Analyze All Reviews", use_container_width=True):
                    ...
                    avg_confidence = results_df['confidence'].mean() if not results_df.empty else 0.0
                    
                    col1.metric("😊 Positive Reviews", f"{positive_count}", 
                                f"{positive_count/len(results_df)*100:.1f}%" if len(results_df) else "0%")
                    col2.metric("😞 Negative Reviews", f"{negative_count}",
                                f"{negative_count/len(results_df)*100:.1f}%" if len(results_df) else "0%")
                    ...
                    with col1:
                        ...
                        if not sentiment_counts.empty:
                            ...
                            st.info("No sentiment results to plot.")
                    ...
                    if not results_df.empty:
                        ...
                        st.info("No analyzed reviews to show or download.")
